...
import { AlertCircle, Wrench, UserPlus } from 'lucide-react';

export const StickyNav: React.FC = () => {
    const [activeTab, setActiveTab] = useState('workshop');

    const tabs = [
        { id: 'challenges', label: 'The Problem', shortLabel: 'Problem', icon: AlertCircle },
    ...
    const scrollToSection = (id: string) => {
        setActiveTab(id);
        const element = document.getElementById(id === 'workshop' ? 'details' : id === 'challenges' ? 'pain-points' : id);
        if (element) {
            const offset = 80;
